...
import gov.dot.fhwa.saxton.carma.guidance.util.matrix.MatrixSolver;
...
public class PolyHoloA implements Filter<Double> {
  ...
  private static final int DEFAULT_POINT_COUNT = 15;
  private static final int  HOLO_MAX_PTS = 16;  //max number of data points needed for Holoborodko filters
  private static ILogger log_ = LoggerManager.getLogger(PolyHoloA.class.getCanonicalName());

  public PolyHoloA(double timeStep) {
    polyPoints_ = DEFAULT_POINT_COUNT;
    timeStep_ = timeStep;
    initialize();
  }
  ...
  public Optional<Signal<Double>> apply(Signal<Double> signal) {
    addRawDataPoint(signal.getData().doubleValue());
    return Optional.of(new Signal<>(getSmoothedValue(), signal.getTimestamp()));
  }
  ...
  /**
   * Sets the number of historical points used for polynomial if newSize >= 3
   * NOTE: After setting this variable reset() should be called before calling apply()
   * 
  ...
  public void setAlternatePolyPoints(int newSize) {
    ...
    if (newSize >= 3) {
      ...
      polyPoints_ = newSize;
      log_.warn("///// Filter is now using an alternate number of polynomial points: " + newSize);
    }
  }

  /**
   * Sets the size of the timestep to use
   * NOTE: After setting this variable reset() should be called before calling apply()
   * 
  ...
  public void initialize() {
    ...
    //since the X values are time since the beginning of the historical buffer, we can pre-compute these sums
    sumX_ = 0.0;
    sumX2_ = 0.0;
    sumX3_ = 0.0;
    sumX4_ = 0.0;
  ...
  private void addRawDataPoint(double rawValue) {
  ...
  private double getSmoothedValue() {
    double x = 0.0;
    double y = 0.0;
    ...
    if (failedPoint_) {
      ...
      log_.debug("Failed smoothing point. Interpolating.");
    ...
    log_.info("getSmoothedValue noise = " + noise);
