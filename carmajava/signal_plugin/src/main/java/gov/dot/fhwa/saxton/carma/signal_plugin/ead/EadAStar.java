...
public class EadAStar implements IEad {
    ...
    protected INodeCollisionChecker     collisionChecker_;          //checking node collisions with NCV trajectory 
    ...
    public EadAStar() {
        ...
        //TODO this is just a placeholder for a concrete instance of the collision checker,
        // which should be initialized after we have a concrete instance
        collisionChecker_ = null;
    }
    ...
    protected Node planCoarsePath(double operSpeed, Node start) throws Exception {
        ...
        double recoveryTime = operSpeed/maxAccel_;
        ...
        //if there is more than one intersection then
        if (numInt > 1) {
            //use the stop bar of the second intersection as the goal for fine solution
            fineGoal = path.get(2); //element 0 is our start node
        //else (only one intersection)
        }else {
            //use the coarse solution's end point as the goal
            fineGoal = path.get(path.size() - 1);
        }
    ...
    protected List<Node> planDetailedPath(Node start, Node goal) throws Exception {
        ...
        fuelCostModel_.setTolerances(new Node(0.5*fineSpeedInc_*fineTimeInc_, 0.51*fineTimeInc_, 0.51*fineSpeedInc_));
