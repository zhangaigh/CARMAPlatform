...
public abstract class NeighborBase implements INeighborCalculator {
    ...
    /**
     * Returns a subset of given candidate nodes after removing nodes that have conflict with NCV
     * @param startNode - the start node of the current plan
     * @param candidateNeighborNodes - a list of candidate neighbor nodes of the start node
     * @return
     */
    protected List<Node> removeConflictingNode(Node startNode, List<Node> candidateNeighborNodes) {
        
        List<Node> validNeighborNodes = new LinkedList<>();
        for(Node node : candidateNeighborNodes) {
            if(!this.collisionChecker_.hasCollision(Arrays.asList(startNode, node))) {
                validNeighborNodes.add(node);
            }
        }
